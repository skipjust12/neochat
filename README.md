...
**Implemented 2026-08-14** (`auth/`, wired into `server.Mux()` -- see `audit.md` finding #1, the last open critical item from the security audit). Every `user_id`/`plan_id` everything above this point relies on -- spend limits, routing tier, conversation history, the moderation log -- now comes from an authenticated caller instead of the request body, closing the gap that let a client obtain unlimited free access to the most expensive catalog model simply by inventing a new `user_id` per request.

- **`Authorization: Bearer <api-key>`** on every `POST /chat`/`POST /chat/stream` call, checked by a new `authenticated` middleware in `server.Mux()` (between `rateLimited` and the handler, so a garbage token is rejected by a cheap per-IP counter before it can drive a database lookup). `chatRequest.UserID`/`PlanID` are now `json:"-"` -- a client-supplied `user_id`/`plan_id` in the body is silently ignored; `decodeChatRequest` fills both in itself from the authenticated identity.
- **`auth.Authenticator`** is the one method (`Authenticate(ctx, token) (Identity, error)`) `server.Server` actually depends on -- deliberately the entire surface, so a future real auth system (JWT/session/OAuth) is a new implementation of this one interface, not a rewrite of the request pipeline. `auth.Store` (`Authenticator` + `IssueKey`) is the fuller surface `auth.PostgresStore`/`auth.InMemoryStore` implement; only `cmd/issuekey` and tests need the issuance half.
- **`auth.PostgresStore`**, backed by a new `api_keys` table (`db/migrations/0004_api_keys.sql`: `token_hash` PK, `user_id`, `plan_id`, `created_at`). Only a token's SHA-256 hash is ever stored -- a leaked table dump doesn't hand out working credentials.
- **Issuance is a manual CLI for now, not a signup flow**: `go run ./cmd/issuekey -user_id=<id> -plan_id=<pro|pro_plus|max>` mints a key and prints it once; there's no login/registration UI yet for a self-serve version to sit behind. Swapping this out later is calling the same `auth.Store.IssueKey` from an HTTP handler instead of a CLI `main` -- the storage layer and `server.go` don't change.
- **Rate limiting is now two-keyed** (2026-08-14, the follow-up this fix unblocked -- `audit.md` finding #2's remaining half). `server.Server.UserRateLimiter` throttles per authenticated `user_id` (`USER_RATE_LIMIT_PER_MINUTE`, default 60/min) alongside the existing per-IP `IPRateLimiter`. They catch opposite halves of the same problem and neither subsumes the other: the per-IP limit is the only one that can throttle a caller with no valid key at all (it runs before authentication), and the per-user limit is the only one that sees a single key driving traffic from many source addresses (no individual IP's counter ever climbs). Middleware order is `recovered` → per-IP → `authenticated` → per-user → handler: the cheapest check that can reject a request runs first, and the per-user one *can't* run earlier, since its key doesn't exist until the token resolves. Both fail open if Redis is unreachable -- a degraded throttle is a bounded loss, a self-inflicted outage isn't.
- **Not done in this pass**: key revocation/expiry (a leaked key today is invalidated by deleting its row by hand). Both rate limits are also still on deliberately generous 60/min placeholders chosen with no measured traffic to tune against; the per-user one is the one worth tightening first, since a human sends single-digit messages per minute while the per-IP limit has to keep headroom for everyone behind one NAT.
...
5. **Cost-based abuse.** A single request with a deliberately bloated context can blow past a dollar limit in one shot. Cost must be estimated from input token count *before* sending, not only after the fact. **Addressed, 2026-08-13/14** (see `audit.md`): `provider.OpenRouterClient.MaxTokens` caps every generation call's *output* side too (`OPENROUTER_MAX_TOKENS`, default 16000), so a single call can no longer run all the way to a catalog model's own `max_output_tokens` ceiling (128000+); `decodeChatRequest` rejects request bodies over `maxRequestBodyBytes` (64KB) before decoding; `server.Server.IPRateLimiter`/`UserRateLimiter` throttle POST `/chat`/`/chat/stream` per client IP *and* per authenticated `user_id` (`RATE_LIMIT_PER_MINUTE`/`USER_RATE_LIMIT_PER_MINUTE`, both default 60/min); and, closing what this item originally flagged as the gap that undermined all of the above, `user_id` is no longer client-supplied at all -- see "Authentication" below and `audit.md` finding #1. An attacker can no longer reset their own spend history by fabricating a fresh `user_id` per request; what's left is the ordinary shape of abuse (a compromised or over-shared API key), not free unlimited access.
...
8. **`/health` doesn't check real dependencies.** `GET /health` (`server.Mux()`) always returns 200, regardless of whether Postgres, Redis, or OpenRouter are actually reachable. Docker's own `HEALTHCHECK` (and any future real orchestrator) will keep routing traffic to an instance whose DB pool is dead, discovering the outage the same way a user does -- via a failed request -- not sooner.
9. **Classifier's safety signal is parsed but never used.** `router.ClassifierOutput.ContentFlags`/`SafetyRiskScore` (`router/types.go`) are decoded from every classify call, but nothing in `router.Route` or `moderation/` reads them -- the field's own doc comment (`TODO: wire up once moderation is actually implemented`) is now stale, since Layer 1 moderation exists and simply never consults these fields. Either fold them into the moderation decision (a second signal already paid for in the same classify call, no extra cost) or drop the dead parsing and the stale TODO.
10. **No structured logging.** Every log line across the codebase is `log.Printf` with free-form text -- no levels, no consistent machine-parseable fields; `user_id`/`request_id` appear ad hoc inside interpolated strings, not as queryable structured keys. Sentry/UptimeRobot (see Infrastructure below) catch crashes and downtime, not "why did this one request behave oddly" -- that still means grepping raw log text by hand.
11. **`cmd/issuekey` has no tests.** It's the entire access-provisioning mechanism today (see "Authentication" above) and currently ships with zero test coverage, even though it's thin glue over already-tested functions (`auth.PostgresStore.IssueKey`, `limits.LoadPlanLimits`) -- cheap to close given how little of it is actually untested logic.
12. **No data retention or deletion story.** `conversation_messages` stores full message text in Postgres indefinitely -- no TTL, no export path, no "delete my data" endpoint -- and there's no privacy policy or ToS yet either. Independent of every access-control fix above: even a perfectly authenticated, rate-limited system is still holding real conversation content with no lifecycle policy around it.
13. **`plan_id` has no connection to real payment.** The "Payments: platega.io / NOWpayments" line in Infrastructure below is a cost-table entry, not integrated code. Today `plan_id` is whatever string an operator typed into `cmd/issuekey` -- upgrading or downgrading a user's plan means hand-editing their `api_keys` row. Expected at this stage (monetization was never claimed to be built, see "Key decisions"), worth stating plainly so it isn't mistaken for a gap in the auth work itself.
14. **Minor: inconsistent handling of a `crypto/rand` failure.** `conversation.NewID` panics if `crypto/rand.Read` fails; `auth.generateToken` (added later, for API key generation) returns an error for the identical failure mode instead. Not a bug -- the failure itself is an OS-level catastrophe either way, vanishingly unlikely in practice -- just a style inconsistency worth normalizing on whichever convention wins, if the code ever gets a general pass for it.
...
- **Authentication: implemented (2026-08-14)**, closing `audit.md` finding #1 (see "Authentication" above) — `POST /chat`/`POST /chat/stream` now require a bearer API key, resolved server-side to the real `user_id`/`plan_id` via a new `auth/` package (`auth.PostgresStore`, `api_keys` table) instead of trusting the request body. Keys are minted by hand via `go run ./cmd/issuekey` — there is still no signup/login flow, so this remains the entire "how does someone get access" story until real onboarding exists.
- **Per-`user_id` rate limiting: implemented (2026-08-14)**, closing the remaining half of `audit.md` finding #2 — the piece that was impossible until authentication landed the same day. `server.Server.UserRateLimiter` (`USER_RATE_LIMIT_PER_MINUTE`, default 60/min) counts one API key across every IP it's used from, which the existing per-IP limiter structurally cannot see; both are wired in `cmd/server` against the same Redis under separate key prefixes. See "Authentication" above for the middleware ordering and why both limiters exist.
