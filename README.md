...
- **Not done in this pass**: key revocation/expiry (a leaked key today is invalidated by deleting its row by hand), and a per-`user_id` rate limit (`IPRateLimiter` is still IP-keyed -- see `audit.md` finding #2's updated status; it could become `user_id`-keyed now that `user_id` is trustworthy, but that's a separate follow-up).
