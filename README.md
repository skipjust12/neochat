...
- **Still not addressed:** no summarization/truncation for a conversation that's genuinely grown past every candidate model's context window — the hard filter now trips on real data instead of stale data, but a long enough conversation still fails outright rather than getting trimmed (see "Context window mismatch" below).
...
7. **Per-request cost accounting, not aggregate.** Log `cost_usd` on every individual API call (classification and generation both), tied to `user_id`, `model`, `timestamp`, from day one. Needed for billing (credit/limit checks), future unit economics, and eval sets. Logging only success/failure without cost is data that's often impossible to reconstruct retroactively — provider pricing changes over time.
   ...
   - **Generation-call persistence: done** (`costlog/`, wired into `server.finalize`) — every completed generation logs one `router.CostLogEntry` via `costlog.Store`, best-effort the same way `Conversations.Append` is (a logging failure doesn't fail the request, since the user already has their answer — it just costs one missing row, not the rolling-window spend `limits.Store` still records correctly for the Thinking+Max cap). `costlog.InMemoryStore` is the same throwaway stand-in as `limits.InMemorySpendStore`/`moderation.InMemoryBlockLog`/`idempotency.InMemoryStore`. `RequestID` is a fresh random ID minted per generation attempt in `server.prepare` (same scheme as `conversation.NewID`), distinct from `conversation_id` (spans a whole thread) and `IdempotencyKey` (client-supplied, optional).
   - **Still not logged: classifier and moderation calls.** `classifier.Classify`/`moderation.Moderate` don't return token usage at all today (just their parsed structured output), even though the `provider.Client.Generate` call underneath each of them does — logging their real per-request cost would need that plumbed out first. `docs/unit-economics.md`'s classifier/moderation cost figures are still model-level estimates, not logged per-request numbers.
...
3. **Context window mismatch.** Switching to a model with a smaller context window may not fit the conversation history. The router treats a candidate's context size as a hard filter (`router.Route`'s `estimatedContextTokens` parameter), now fed a real server-computed estimate instead of an unverified client-supplied one (see "Tokenizer / context estimation" above) — but there's still no unified summarization/truncation mechanism: a conversation that's genuinely outgrown every candidate's context window just fails the hard filter outright instead of being trimmed before sending.
