...
- Needed the response to actually be streamed first — that part's now done (`POST /chat/stream`, see "Streaming" below) — but nothing reads the stream mid-flight to moderate it yet; Layer 2 itself (scanning `StreamChunk`s as `handleStream` relays them) is still unbuilt.
...
- Both checks are async goroutines, never sequential steps. (True today for classify+moderate; will also apply to generate+Layer 2 once Layer 2 exists.)
- Cost on flagged requests isn't always zero once Layer 2 exists — generation may partially complete in parallel with the input check. Accepted tradeoff for UX (the alternative is DPI-style latency on every request). Not a concern yet for Layer 1 alone: it gates generation outright, so a flagged request costs only the classify+moderate calls, nothing more.
...
- **Not done yet:** Layer 2 (output) moderation, which needs exactly this incremental stream to scan against — see "Moderation" above.
...
8. ~~Implement Layer 1 (input) moderation~~ — done (`moderation/`). ~~Blocked on `/chat` not streaming~~ — `POST /chat/stream` now exists (see "Streaming"), so this is unblocked: Layer 2 still needs building, scanning `StreamChunk`s as `handleStream` relays them instead of a response delivered in one shot.
