...
Дата: 2026-08-13 (обновлено 2026-08-13 — закрыты №2 частично, №3, №4, №9, №10; второй проход добавил и закрыл №11–№14; третий — №15–№17; 2026-08-14 — закрыты №1 и, как следствие, №5, а также оставшаяся половина №2; №6 закрыт отдельным решением — принят как риск, не фикс кодом. См. пометки "✅ Исправлено"/"Закрыто (принято как риск)" в каждом разделе).
...
**Не закрыто этим фиксом:** отзыв ключей (нет `revoked_at`/эндпоинта отзыва — сейчас единственный способ обезвредить утёкший ключ — вручную удалить его строку из `api_keys`); срок действия токенов (не истекают). Per-`user_id` rate limiting, который этот фикс делал возможным, но сам не реализовывал, добавлен отдельно 2026-08-14 — см. находку №2 ниже, там же и закрыт полностью.
...
### 6. Модерация видит только последнее сообщение, не всю историю — закрыто как продуктовое решение

**Где:** `server/server.go` (комментарий у `server.handle` явно объясняет, что это сознательное решение), README "Moderation".

**Суть:** многоходовые попытки обойти модерацию (по частям в разных репликах, каждая по отдельности безобидная) не ловятся Layer 1, поскольку он смотрит только на последнее сообщение, а не на окно/всю историю диалога.

**Решение (2026-08-14): сознательно не расширять модерацию на историю, не открытая находка.** Раньше этот пункт держал рекомендацию прогонять модерацию по недавнему окну диалога, а не только по последнему сообщению — вычеркнуто по прямому решению автора продукта, той же логикой, что и отказ от Layer 2 выше: генерирующая модель уже несёт собственную вендорскую safety-настройку и в подавляющем большинстве случаев сама откажется от джейлбрейка, даже если Layer 1 его не поймал, — так что окно/история ловит только узкий случай "разбито на безобидные с виду куски", а стоимость (растущая с длиной разговора, при живом трафике не разовая) сейчас не оправдана никакими данными об инцидентах.

**Осознанный компромисс, а не слепое пятно:** если джейлбрейк всё же соберётся из нескольких сообщений, `moderation.BlockLog` эту попытку не увидит — либо модель откажет сама (без записи в лог, отказ виден только в её ответе), либо запрос пройдёт. То есть при таком сценарии нет своей записи о том, что кто-то пытался, только надежда на вендорский бэкстоп. Разумно поменять эту оценку, когда появятся вживую наблюдаемые случаи (реальный инцидент, не гипотетический), а не заранее.
...
| 6 | Модерация не покрывает всю историю (многоходовый обход) | Средний | Закрыто (принято как риск) — полагаемся на vendor-side safety моделей, см. раздел | `server/server.go` |
